...
class Metasploit3 < Msf::Post
  ...
  def initialize(info = {})
    ...
    register_advanced_options([
      ...
      OptString.new('ReverseListenerComm',
        [false, 'The comms channel to use (same as other exploits)', nil])
    ], self.class)
  ...
  def run
    ...
    if datastore['PAYLOAD_OVERRIDE']
        if payload_name =~ /python/
            platform = 'python'
        elsif payload_name =~ /linux/
            platform = 'linux'
        elsif payload_name =~ /windows/
            platform = 'win'
        end
    end
    vprint_status("Upgrade payload: #{payload_name}")
    vprint_status("Platform is :#{platform}")
  ...
  def create_multihandler(lhost, lport, payload_name)
    ...
    if datastore['ReverseListenerComm'] != nil
        vprint_status("Using ReverseListenerComm : #{datastore['ReverseListenerComm']}")
        pay.datastore['ReverseListenerComm'] = datastore['ReverseListenerComm']
    end
